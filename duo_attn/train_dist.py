...
#Removed convert_image_tensor_to_llava_tokens, which was unused
...
def train(
    args, model : LlavaForConditionalGeneration, rank, world_size, train_dataloader, optimizer, scheduler, resume_step, processor, mesh
):
    ...
    while True:
        ...
        for step, batch in enumerate(train_dataloader):
            print(f"step {step} batch size: {len(batch['input_ids'])}, batch keys: {batch.keys()}")
            ...
            if (isinstance(model, LlavaForConditionalGeneration)):
                ...
                #This seems very inefficient because we are placing the full input on each GPU to embed it
                #Ideally, we could split the input and embed only the relavent part.
                dist_inputs_ids = distribute_tensor(input_ids, device_mesh=mesh) #, placements=[Replicate()])
                #dist_pixel_values = pixel_values
                dist_pixel_values = distribute_tensor(pixel_values, device_mesh=mesh) # placements=[Replicate()])

                #Text embeddings
                inputs_embeds = model.get_input_embeddings()(dist_inputs_ids)
                inputs_embeds = inputs_embeds.to(f"cuda:{local_rank}") 
                #Image embeddings
                #Want to use the same strategy as was used to process images
                image_features = get_image_features(model, dist_pixel_values, vision_feature_select_strategy=processor.vision_feature_select_strategy)
                #image_features = distribute_tensor(image_features, device_mesh=mesh)
                ...
                ).unsqueeze(0).to(f"cuda:{local_rank}")

                outputs = model.language_model.model(
                    #input_ids=input_ids[:, seq_parallel_chunk_start:seq_parallel_chunk_end], #.to("cuda:0"),
                    attention_mask=attention_mask[:, seq_parallel_chunk_start:seq_parallel_chunk_end], #.to("cuda:0"),
                    position_ids=position_ids, #.to("cuda:0"),
                    inputs_embeds=inputs_embeds[:, seq_parallel_chunk_start:seq_parallel_chunk_end],
                )
